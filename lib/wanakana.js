...
wanakana._onInput = function(event) {
  var androidChromeFix, input, newCursor, newText, normalizedInputString, startingCursor, startingLength, _ref;
  input = event.target;
  ...
  if (normalizedInputString !== newText) {
    input.value = newText;
    androidChromeFix = (_ref = startingCursor === 0) != null ? _ref : {
      1: 0
    };
    newCursor = startingCursor - startingLength + newText.length + androidChromeFix;
    return input.selectionStart = input.selectionEnd = newCursor;
  }
};
